...
const BudgetSummary = ({ balance, usagePercentage }: BudgetSummaryProps) => {
  ...
  const barWidth = Math.min(roundedPercentage, 100);
  ...
  return (
    <div className="relative z-10 flex flex-col items-center gap-4 py-8">
      <div className="flex flex-col gap-1">
        ...
        <p 
          className={`text-5xl font-bold transition-colors duration-300 ${
            isPositive ? "text-blue-700" : "text-red-600"
          }`}
        >
      ...
      <div className="flex w-full max-w-48 flex-col gap-1">
        <div className="h-1.5 w-full rounded-2xl bg-gray-700 overflow-hidden">
          <div
            className={`h-full transition-all duration-500 ease-out ${
              isPositive ? "bg-blue-600" : "bg-red-600"
            }`}
